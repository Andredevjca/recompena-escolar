...
public class RepositorioRecompensas(Banco banco, EstadoDemonstracao estado, IRepositorioPainel painel) : IRepositorioRecompensas
{
    ...
    {
        if (banco.ModoDemonstracao) { await estado.Semaforo.WaitAsync(); try { var indice = estado.Painel.Boletins.FindIndex(x => x.Id == boletimId); if (indice < 0) throw new InvalidOperationException("Recompensa não encontrada."); estado.Painel.Boletins[indice] = estado.Painel.Boletins[indice] with { Pago = true }; } finally { estado.Semaforo.Release(); } return; }
